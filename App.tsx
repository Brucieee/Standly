...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (link) {
      link.href =
        "https://qizxqbaylaaatskyqzpl.supabase.co/storage/v1/object/public/Standly/assets/logo.png";
    } else {
  ...
  const loadData = async () => {
    try {
      ...
      const mappedStandups = (standupsData || []).map((s: any) => {
        ...
        const standupUser =
          (standupUserRaw
            ? mapProfileToUser(standupUserRaw)
            : mappedUsers.find((u) => u.id === (s.user_id || s.userId))) ||
          unknownUser;
        ...
        const comments = (s.comments || [])
          .map((c: any) => {
            ...
            const commentUser =
              (commentUserRaw
                ? mapProfileToUser(commentUserRaw)
                : mappedUsers.find((u) => u.id === (c.user_id || c.userId))) ||
              unknownUser;
            return {
              ...c,
        ...
        const reactions = (s.reactions || []).map((r: any) => {
          ...
          const reactionUser =
            (reactionUserRaw
              ? mapProfileToUser(reactionUserRaw)
              : mappedUsers.find((u) => u.id === (r.user_id || r.userId))) ||
            unknownUser;
          return {
            ...r,
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "standup_comments" },
        () => loadData(),
      )
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "standup_reactions" },
        () => loadData(),
      )
      .on(
        "postgres_changes",
  ...
  const handleRegister = async (
    ...
    try {
      const { user, session } = await apiAuth.signUp(
        email,
        password,
        name,
        role,
  ...
  const handleCalendarDateClick = (dateStr: string) => {
    const existing = state.standups.find(
      (s) => s.userId === state.currentUser?.id && s.date.startsWith(dateStr),
    );
    if (existing) {
  ...
  const handleSaveStandup = async (data: {
    date: string;
    yesterday: string;
    today: string;
    blockers: string;
    ...
    try {
      ...
      toast.success(
        editingStandup ? "Standup updated!" : "Standup posted!",
        {
          description: `For ${data.date}`,
        },
      );
    } catch (error) {
      console.error("Failed to save standup", error);
      toast.error("Failed to save standup. Please try again.");
    }
  };
    ...
    try {
      ...
      setState((prev) => ({
        ...
        standups: prev.standups.map((s) =>
          s.id === standup.id
            ? { ...s, views: [...(s.views || []), state.currentUser!.id] }
            : s,
        ),
      }));
    } catch (error) {
  ...
  const handleReact = async (standupId: string, reactionType: string) => {
    ...
    try {
      ...
      if ((error as any).code === "42501") {
        alert(
          'Permission denied: You need to enable RLS policies in Supabase for "standup_reactions".',
        );
      } else {
  ...
  const handleComment = async (
    ...
    try {
      ...
      if ((error as any).code === "42501") {
        alert(
          'Permission denied: You need to enable RLS policies in Supabase for "standup_comments".',
        );
      } else {
  ...
  const handleEditComment = async (commentId: string, text: string) => {
    ...
    try {
      ...
      if ((error as any).code === "42501") {
        alert(
          'Permission denied: You need to enable UPDATE RLS policies for "standup_comments".',
        );
      } else {
  ...
  const handleDeleteComment = async (commentId: string) => {
    ...
    try {
      ...
      if ((error as any).code === "42501") {
        alert(
          'Permission denied: You need to enable DELETE RLS policies for "standup_comments".',
        );
      } else {
  ...
  const handleSaveDeadline = async (data: Partial<Deadline>) => {
    ...
    try {
      if (editingDeadline) {
        ...
        setState((prev) => ({
          ...prev,
          deadlines: prev.deadlines.map((t) =>
            t.id === editingDeadline.id ? { ...t, ...data } : t,
          ),
        }));
      } else {
      ...
      const deadlineTitle = (editingDeadline?.title || data.title) as string;
      const deadlineDate = new Date(
        (editingDeadline?.dueDate || data.dueDate) as string,
      ).toLocaleDateString();

      toast.success(
        editingDeadline ? "Deadline updated!" : "Deadline set!",
        {
          description: `${deadlineTitle} is due on ${deadlineDate}`,
        },
      );
    } catch (error) {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        try {
          ...
          alert(
            `Failed to delete deadline: ${error.message || "Unknown error"}. Please check console.`,
          );
        }
      },
    });
      ...
      if (editingLeave) {
        ...
        setState((prev) => ({
          ...
          leaves: prev.leaves.map((l) =>
            l.id === editingLeave.id ? { ...l, ...data } : l,
          ),
        }));
      } else {
        const { data: newLeaveData, error } = await supabase
          .from("leaves")
          .insert({
            user_id: state.currentUser.id,
            ...dbData,
  ...
  const handleSaveHoliday = async (date: string, name: string, id?: string) => {
    try {
      ...
      if ((error as any).code === "42501") {
        alert(
          'Permission denied: Only admins can add holidays. Please ensure your account has "is_admin" set to TRUE in the database.',
        );
      } else {
  ...
  const viewingStandupObject = historyViewingStandupId
    ? state.standups.find((s) => s.id === historyViewingStandupId)
    : null;
  ...
  return (
    <>
      ...
      <StandupModal
        ...
        onDelete={
          editingStandup
            ? () => handleDeleteStandup(editingStandup.id)
            : undefined
        }
